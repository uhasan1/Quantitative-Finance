...
def clean_data(df, dseries_old, dseries):
    ...
    df[dseries] = df[dseries].str[0:2].replace(r'\b%s\b' % 'THE', '')  
...
def parse_ccc_abb(file_std, file_ccc, df, dseries, dseries2):
    ...
    # Drop empty cells 
    df = df[df[dseries2] != '']
    # If the word 'INTERNATIONAL' appears from 3rd word onwards, drop all subsequent words 
    df[dseries2][df[dseries2].str.split().str[2:].str.join(' ').str.contains('INTERNATIONAL')] = (df[dseries2]
    [df[dseries2].str.split().str[2:].str.join(' ').str.contains('INTERNATIONAL')].str.split('INTERNATIONAL').str[0] + 'INTERNATIONAL')
...
def fuzzy_match():        
    ...
    for row in df_fuzzy_match.itertuples():
        ...
        condition1 = (df_customers[GDWH_combined_columns[3]].str.split().str[0:2].str.join(' ') == (r'%s' % ' '.join(row[2].split()[0:2])))
        condition2 = (df_customers[GDWH_combined_columns[3]].str.find(r'%s' % ' '.join(row[2].split()[0:2])) >= 0)
...
def exact_match_modified_ccc(df, dseries, dseries_co, dseries_ccif, dseries_ratio, dseries_tag, df_cust):
    ...
    df_match['TEMP'] = df_match[dseries].str.replace(' ', '')
    del df_match[dseries]
