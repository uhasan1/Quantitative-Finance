...
TRICS_columns = ['COUNTRY_FROM_2', 'COUNTRY_TO_2', 
                 'REMITTING_BANK_JP_BANK_GRP_2', 'BENEFICIARY_BANK_JP_BANK_GRP_2', 
                 'REMITTER_ENGLISH', 'REMITTER_C_CIF', 
                 'BENEFICIARY_ENGLISH', 'BENEFICIARY_C_CIF']
TRICS_nayose_columns = ['COUNTRY_FROM', 'COUNTRY_TO', 'COUNTRY_FROM_2', 'COUNTRY_TO_2', 
                        'REMITTING_BANK_JP_BANK_GRP_2', 'BENEFICIARY_BANK_JP_BANK_GRP_2', 
                        'REMITTER_ENGLISH', 'REMITTER_C_CIF', 'REMITTER_C_CIF_2', 
                        'BENEFICIARY_ENGLISH', 'BENEFICIARY_C_CIF', 'BENEFICIARY_C_CIF_2',
                        'REMITTING_ACCOUNT', 'BENEFICIARY_ACCOUNT', 'SENDERS_REFERENCE_NUMBER',
                        'REMITTER_NAYOSE_FLG', 'BENEFICIARY_NAYOSE_FLG']
TRICS_cust_columns = ['REMITTER_ENGLISH', 'BENEFICIARY_ENGLISH']
TRICS_ccif_columns = ['REMITTER_C_CIF', 'REMITTER_C_CIF_2', 'BENEFICIARY_C_CIF', 'BENEFICIARY_C_CIF_2']
TRICS_combined_columns = ['COMBINED_CUST', 'COMBINED_CUST_NEW', 'COMBINED_CUST_CCC', 'COMBINED_CUST_CO']
past_remit = ['COUNTRY_FROM_2', 'REMITTER_ENGLISH', 'REMITTER_NAYOSE_FLG', 'PYTHON_COMPANY_A_BEST_MATCH', 'PYTHON_CCIF_A_BEST_MATCH',
              'PYTHON_A_HIGHEST_RATIO', 'PYTHON_A_UPDATE_TAG', 'UPDATE_FLAG_REMIT']
past_bene = ['COUNTRY_TO_2', 'BENEFICIARY_ENGLISH', 'BENEFICIARY_NAYOSE_FLG', 'PYTHON_CCIF_B_BEST_MATCH', 'PYTHON_COMPANY_B_BEST_MATCH', 
...
def return_trics_data(TRICS_columns, TRICS_file, chunk_size, offset):
...
def return_other_tables(past_remit_file, past_bene_file):
    ...
    # Import DW_REMITTANCE from mySQL server
    ref_no_column_list = ','.join(map(str, [GDWH_ccif_columns[0]] + [GDWH_ccif_columns[2]]))
    df_ref_no = pd.read_sql('SELECT %s FROM dw_remittance WHERE %s IS NOT NULL;' % (ref_no_column_list, GDWH_ccif_columns[2]), con = conn)
    ...
    # Import past REMITTER updates
    past_remit_column_list = ','.join(map(str, past_remit))
    ...
    return df_gdwh, df_iso, df_ref_no, df_dw, df_past_remit, df_past_bene
...
def nayose_ccif_update(df): 
    ...
    del df[GDWH_ccif_columns[0]], df[GDWH_ccif_columns[2]]
    ...
    # Update REMITTER_NAYOSE_FLAG and BENEFICIARY_NAYOSE_FLAG to 1 for all matched records
...
def total_match():        
    ...
    # Update past REMITTER and BENEFICIARY results against current dataset
    df_exact_match = pd.merge(df_exact_match, df_past_remit, how = 'left', on = ['COUNTRY', 'COMBINED_CUST'])
    df_exact_match = pd.merge(df_exact_match, df_past_bene, how = 'left', on = ['COUNTRY', 'COMBINED_CUST'])
    df_exact_match['COMPANY_BEST_MATCH'][df_exact_match['UPDATE_FLAG_REMIT'] == 'Y'] = df_exact_match[df_exact_match['UPDATE_FLAG_REMIT'] == 'Y']['PYTHON_COMPANY_A_BEST_MATCH']
    ...
    df_exact_match['CCIF_BEST_MATCH'][df_exact_match['UPDATE_FLAG_REMIT'] == 'Y'] = df_exact_match[df_exact_match['UPDATE_FLAG_REMIT'] == 'Y']['PYTHON_CCIF_A_BEST_MATCH']
...
def consol():
    ...
    del df_main['REMITTING_ACCOUNT'], df_main['BENEFICIARY_ACCOUNT'], df_main['SENDERS_REFERENCE_NUMBER']
    ...
    # Create an additional flag for UPDATE_FLAG_REMIT and update based on different status
    ...
    df_main_1['UPDATE_FLAG_REMIT'][df_main_1['UPDATE_FLAG_REMIT'] == ''] = np.where((df_main_1['CCC1'][df_main_1['UPDATE_FLAG_REMIT'] == ''] != 
                                                                                     df_main_1['CCC2'][df_main_1['UPDATE_FLAG_REMIT'] == '']) & 
